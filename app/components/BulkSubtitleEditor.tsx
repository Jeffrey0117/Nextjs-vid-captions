...
export default function BulkSubtitleEditor({ isOpen, onClose, videoUrl }: BulkSubtitleEditorProps) {
  ...
  const [showTranslationModal, setShowTranslationModal] = useState(false); // 翻譯選擇視窗
  ...
  const translateAllSubtitles = async (apiEndpoint: string, serviceName: string) => {
    ...
    try {
      // 收集所有要翻譯的文字
      const textsToTranslate = segments
        .map(segment => segment.text || segment.translatedText)
      ...
      const response = await fetch(apiEndpoint, {
        ...
        body: JSON.stringify({ texts: textsToTranslate }), // 注意：改成 texts 陣列
      });
      ...
      if (data.success && data.translatedTexts && Array.isArray(data.translatedTexts)) {
        ...
        segments.forEach((segment, segmentIndex) => {
          const originalText = segment.text || segment.translatedText;
          if (originalText && originalText.trim()) {
            const translatedText = data.translatedTexts[textIndex];
            if (translatedText) {
              console.log(`字幕 ${segmentIndex + 1}:`, {
                原文: originalText,
                翻譯結果: translatedText
              });
