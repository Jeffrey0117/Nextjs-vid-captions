...
export default function EditorProPage() {
  ...
  const handleExportVideo = async () => {
    ...
    try {
      ...
      // 根據選擇的渲染方式調用不同的 API
      const apiEndpoint = renderMethod === 'canvas' ? '/api/render-video-v2' : '/api/burn-subtitles';
    ...
    <div className="h-screen flex flex-col bg-gray-950 text-white">
      ...
      <header className="h-10 border-b border-gray-800 flex items-center px-3 gap-1.5 bg-gray-900">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            <option value="canvas">Canvas 渲染 (推薦)</option>
            <option value="ffmpeg">FFmpeg 渲染</option>
          </select>
        </div>
