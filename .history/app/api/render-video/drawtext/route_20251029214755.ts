...
export async function POST(request: Request) {
  try {
    ...
    if (videoPath) {
      // 首先嘗試在 public 目錄找影片
      const publicVideoPath = path.join(process.cwd(), "public", videoPath);
      
      if (fs.existsSync(publicVideoPath)) {
        finalVideoPath = publicVideoPath;
        console.log("📁 Using video from public:", finalVideoPath);
      } else {
        // 然後嘗試在 temp 目錄找影片
        finalVideoPath = path.join(tempDir, videoPath);
        console.log("📁 Trying video from temp:", finalVideoPath);
        
        if (!fs.existsSync(finalVideoPath)) {
          console.log("❌ Video file not found in both public and temp directories");
          console.log("❌ Checked paths:", publicVideoPath, finalVideoPath);
          return NextResponse.json(
            { error: "Video file not found", publicPath: publicVideoPath, tempPath: finalVideoPath },
            { status: 404 }
          );
        }
      }
