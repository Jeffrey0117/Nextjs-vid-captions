...
export async function POST(request: Request) {
  ...
  try {
    ...
    const ffmpegCommand = process.platform === 'win32'
      ? `ffmpeg -i "${finalVideoPath.replace(/\\/g, '/')}" -framerate ${videoFps} -i "${framesDir.replace(/\\/g, '/')}/subtitle_%06d.png" -i "${framesDir.replace(/\\/g, '/')}/subtitle_%06d.png" -filter_complex "[1:v][0:v]overlay=0:0[v]" -map "[v]" -map 0:a -c:v libx264 -c:a copy -r ${videoFps} "${outputPath.replace(/\\/g, '/')}"`
      : `ffmpeg -i "${finalVideoPath}" -framerate ${videoFps} -i "${framesDir}/subtitle_%06d.png" -filter_complex "[1:v][0:v]overlay=0:0[v]" -map "[v]" -map 0:a -c:v libx264 -c:a copy -r ${videoFps} "${outputPath}"`;
